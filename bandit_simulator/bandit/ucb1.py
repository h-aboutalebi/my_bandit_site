# This class implements the ucb1 algorithm
...
class ucb1_algorithm():
    ...
    def __init__(self, number_of_arms, initial_values, c_value, reward_matrix):
    ...
    # This function puts together the components of ucb1 (choose_arm,update_value) and runs the whole algorithm
    def run(self):
    ...
    # This function choose arms based on the ucb1 algorithm by considering the variance
    def choose_arm(self):
        new_value = self.ucb1_policy_value()  # before choosing an arm, we modify mean value to consider variances according to ucb1
    ...
    # This function modifies the average value according to ucb1 algorithm so as to consider variance
    def ucb1_policy_value(self):
        new_value = []
        arm_number = 0
        for mean in self.value_arms:
            variance = self.compute_ucb1_variance(arm_number)
            new_value.append(mean + self.c_value*variance)
            arm_number += 1
        return new_value

    # This function will compute the value of the variance for ucb1:
    def compute_ucb1_variance(self, arm_number):
        if (self.number_of_times_arm_chosen[
            arm_number] == 0):  # This is for the initial case where the number of times the arm is chosen is zero
            return 10000
        else:
            return math.sqrt((2 * math.log(len(self.chosen_arm_history))) / self.number_of_times_arm_chosen[arm_number])
